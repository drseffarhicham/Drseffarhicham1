...
export default function Testimonials() {
  ...
  const realReviews: Testimonial[] = [
    ...
    },
    {
      ...
      date: t('testimonials.items.5.date'),
      verified: true,
  ...
  return (
    <section id="temoignages" className="py-24 bg-transparent overflow-hidden relative">
      {/* Decorative background elements */}
      <div className="absolute top-0 end-0 w-96 h-96 bg-clinic-pink-50 rounded-full blur-3xl opacity-50 -me-48 -mt-24 -z-10"></div>
      <div className="absolute bottom-0 start-0 w-96 h-96 bg-clinic-green-50 rounded-full blur-3xl opacity-50 -ms-48 -mb-24 -z-10"></div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16 relative">
          <div className="inline-flex items-center px-4 py-2 bg-white shadow-sm border border-clinic-green-100 rounded-full text-sm font-bold mb-6 text-clinic-green-700">
            {syncing ? (
              <RefreshCw className="h-4 w-4 me-2 animate-spin text-clinic-green-500" />
            ) : (
                <CheckCircle2 className="h-4 w-4 me-2 text-green-500" />
              )}
            {t('testimonials.badge_sync')}
          </div>
          
          <h2 className="text-2xl md:text-5xl font-extrabold text-gray-900 mb-6 tracking-tight">
            {t('testimonials.title_start')} <span className="text-transparent bg-clip-text bg-gradient-to-r from-clinic-green-700 to-clinic-pink-500 font-black">{t('testimonials.title_highlight')}</span>
          </h2>
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center py-20 min-h-[400px]">
            ...
            <p className="mt-6 text-gray-500 font-medium animate-pulse">{t('testimonials.loading')}</p>
          </div>
        ) : (
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {testimonials.map((testimonial, index) => {
              const cardContent = (
                <>
                  <div className="absolute -top-4 -end-2 w-16 h-16 bg-gradient-to-br from-clinic-green-50 to-clinic-pink-50 rounded-full blur-xl opacity-0 group-hover:opacity-100 transition-opacity"></div>
                  
                  <Quote className="absolute top-8 end-8 h-12 w-12 text-gray-100 group-hover:text-clinic-green-50 transition-colors duration-500 -z-10" />

                  <div className="flex items-center gap-1 mb-6">
                    {[...Array(testimonial.rating)].map((_, i) => (
                      <Star key={i} className="h-5 w-5 text-amber-400 fill-current drop-shadow-sm" />
                    ))}
                    <span className="ms-2 text-sm font-bold text-gray-400 uppercase tracking-tighter">{t('testimonials.verified')}</span>
                  </div>
        ...
        <div className="mt-20 text-center">
          ...
          >
            <span className="relative z-10 flex items-center">
              <ExternalLink className="h-5 w-5 me-3 group-hover:rotate-12 transition-transform" />
              {t('testimonials.view_all')}
            </span>
            <div className="absolute top-0 -start-[100%] w-[100%] h-full bg-gradient-to-r from-transparent via-gray-100 to-transparent group-hover:start-[100%] transition-all duration-1000"></div>
          </a>
        </div>
      </div>
    </section>
